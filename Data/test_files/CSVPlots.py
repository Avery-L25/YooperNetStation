...
def splitDF() -> dict[str, pd.DataFrame]:
    data = {}
    try:
        for i in df['folder'].unique():
            _, mini_df = df.groupby(df['folder'] == i)
            data[i] = mini_df[1]
            print(i)
    except KeyError:
...
getDF(os.path.join(os.path.dirname(__file__),"LEDS_07-08_11.csv"))
# endregion



def plotCSV2Much(data_frame=df, avg_color=False):
...
def plotColorComparison(data_frame=df):
    'Plot aurora checking data from csv file'
...
def plotMask(data_frame=df,SubTitle=''):
    global fig, ax, ax0, ax1, ax2, plot
    data_frame = checkDF(data_frame)
    # Get figure
